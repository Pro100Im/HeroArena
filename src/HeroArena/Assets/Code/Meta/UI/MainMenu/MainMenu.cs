    ...
    public class MainMenu : MonoBehaviour
    {
        ...
        private void OnAnyButtonPress(InputAction.CallbackContext context)
        {
            if (context.canceled)
            {
                ...
                DOTween.To(() => _intro.style.opacity.value, x => _intro.style.opacity = x, 0f, _fadeDuration);

                DOTween.To(() => _mainMenu.style.opacity.value, x => _mainMenu.style.opacity = x, 1f, _fadeDuration);
        ...
        {
            DOTween.To(() => _mainMenu.style.opacity.value, x => _mainMenu.style.opacity = x, 0f, _fadeDuration);
            _quickMatchButton.pickingMode = PickingMode.Ignore;
            _exitButton.pickingMode = PickingMode.Ignore;

            DOTween.To(() => _searchingPopUp.style.opacity.value, x => _searchingPopUp.style.opacity = x, 1f, _fadeDuration);
        ...
        private void CancleQuickMatch()
        {
            ...
            DOTween.To(() => _searchingPopUp.style.opacity.value, x => _searchingPopUp.style.opacity = x, 0f, _fadeDuration);
            _cancelSearchingButton.pickingMode = PickingMode.Ignore;
        }
